...
function M.check()
	...
	if sshfs == 1 then
		...
		vim.health.error("sshfs not found in $PATH", {
	...
	elseif vim.fn.has("linux") == 1 then
		...
		if vim.fn.filereadable("/dev/fuse") == 1 then
			...
			vim.health.warn("/dev/fuse not found — kernel FUSE may not be available", { "Check: modprobe fuse" })
		end
	end
	...
	if vim.fn.has("nvim-0.9") == 1 then
		...
		vim.health.error(
			"Neovim >= 0.9 required (current: " .. vim.version().major .. "." .. vim.version().minor .. ")"
		)
	end
