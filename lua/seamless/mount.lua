	...
	while true do
		...
		if sshpass_cmd then
			local pw_check_cmd = {
				...
				"BatchMode=no",
				"-o",
				"ConnectTimeout=" .. (config.ssh.preflight_timeout or 5),
				"-o",
			...
			if pw_exit == 5 then
				...
				local pass = vim.fn.inputsecret("Incorrect password. SSH password for " .. host_label .. ": ")
				if pass == "" then
					mount_in_progress[key] = nil
					return nil, "" -- user cancelled, not an error
				end
...
function M.unmount(host)
	...
	if ok then
		-- If CWD was inside the mount, move to ~ to prevent cwd() nil crash.
